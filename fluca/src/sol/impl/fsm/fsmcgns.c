...
static PetscErrorCode FieldNameToVec_Private(Sol sol, const char field_name[], Vec *v)
{
  PetscBool isfsm;
  PetscInt  d;
  PetscBool flg;

  PetscCall(PetscObjectTypeCompare((PetscObject)sol, SOLFSM, &isfsm));

#define Match(name, vec) \
  do { \
    PetscCall(PetscStrcmp(field_name, name, &flg)); \
    if (flg) { \
      *v = vec; \
      PetscFunctionReturn(PETSC_SUCCESS); \
    } \
  ...
  PetscFunctionBegin;
  for (d = 0; d < 3; ++d) Match(velnames[d], sol->v[d]);
  Match(presname, sol->p);

  if (isfsm) {
    Sol_FSM *fsm = (Sol_FSM *)sol->data;

    for (d = 0; d < 3; ++d) {
      Match(intervelnames[d], fsm->v_star[d]);
      Match(convecnames[d], fsm->N[d]);
      Match(prevconvecnames[d], fsm->N_prev[d]);
      Match(facevelnames[d], fsm->fv);
      Match(faceintervelnames[d], fsm->fv_star);
    }
    Match(preshalfname, fsm->p_half);
    Match(prevpreshalfname, fsm->p_half_prev);
    Match(prescorrecname, fsm->p_prime);
  }
  SETERRQ(PetscObjectComm((PetscObject)sol), PETSC_ERR_ARG_UNKNOWN_TYPE, "Unknown field name: %s", field_name);

#undef Match
}

static PetscErrorCode DMStagLoadCellCenteredSolution_Private(DM dm, Vec v, int file_num, int base, int zone, int solution, int field, CGNS_ENUMT(DataType_t) data_type)
{
...
PetscErrorCode SolLoadCGNS_FSM(Sol sol, PetscInt file_num)
{
  ...
  for (field = 1; field <= num_fields; ++field) {
    ...
    char                   field_name[CGIO_MAX_NAME_LENGTH + 1];
    Vec                    v;

    CGNSCall(cg_field_info(file_num, base, zone, solution, field, &data_type, field_name));
    PetscCall(FieldNameToVec_Private(sol, field_name, &v));
    if (iscart) PetscCall(DMStagLoadCellCenteredSolution_Private(dm, v, file_num, base, zone, solution, field, data_type));
  }
