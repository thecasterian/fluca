...
PetscErrorCode SolView_FSMCGNS(Sol sol, PetscViewer viewer) {
    ...
    if (!cgns->output_times)
        PetscCall(PetscSegBufferCreate(sizeof(PetscReal), 20, &cgns->output_times));
    if (!cgns->output_steps)
        PetscCall(PetscSegBufferCreate(sizeof(size_t), 20, &cgns->output_steps));
