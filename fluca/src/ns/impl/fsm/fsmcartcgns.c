...
PetscErrorCode NSViewSolution_FSM_Cart_CGNS_Internal(NS ns, PetscViewer viewer)
{
  ...
  *step_slot = step;
  ...
  for (d = 0; d < dim; ++d) {
    PetscCall(DMStagWriteCellCenteredSolution_Private(dm, fsm->v[d], cgv->file_num, cgv->base, cgv->zone, solution, velnames[d]));
  ...
  PetscCall(DMStagWriteCellCenteredSolution_Private(dm, fsm->p, cgv->file_num, cgv->base, cgv->zone, solution, presname));
...
PetscErrorCode NSLoadSolutionCGNS_FSM_Cart_Internal(NS ns, PetscInt file_num)
{
  ...
  for (d = 0; d < dim; ++d) {
    PetscCall(DMStagLoadCellCenteredSolution_Private(dm, fsm->v[d], file_num, base, zone, solution, velnames[d]));
  ...
  PetscCall(DMStagLoadCellCenteredSolution_Private(dm, fsm->p, file_num, base, zone, solution, presname));
