...
PetscErrorCode FlucaMapCreate(MPI_Comm comm, FlucaMap *map, PetscErrorCode (*hash)(PetscObject, PetscInt *), PetscErrorCode (*eq)(PetscObject, PetscObject, PetscBool *))
{
  FlucaMap m;
...
PetscErrorCode FlucaMapInsert(FlucaMap map, PetscObject key, PetscObject value)
{
  ...
  kv->key   = key;
  kv->value = value;
  if (map->hash) PetscCall(map->hash(key, &kv->hash));
  else kv->hash = (uintptr_t)key % PETSC_INT_MAX;
...
PetscErrorCode FlucaMapRemove(FlucaMap map, PetscObject key)
{
  ...
  if (map->hash) PetscCall(map->hash(key, &hash));
  else hash = (uintptr_t)key % PETSC_INT_MAX;
  ...
  for (kv = map->buckets[index].front; kv; kv = kv->next) {
    if (map->eq) PetscCall(map->eq(kv->key, key, &eq));
    else eq = kv->key == key;
    if (eq) {
