...
PetscErrorCode FlucaFinalize(void) {
    ...
    PetscCheck(FlucaInitializeCalled, PETSC_COMM_SELF, PETSC_ERR_ARG_WRONGSTATE,
               "FlucaInitialize() must be called before FlucaFinalize()");
    ...
    if (FlucaBeganPetsc) {
        ...
        PetscCheck(
            !flg, PETSC_COMM_SELF, PETSC_ERR_LIB,
            "PetscFinalize() has already been called, even though PetscInitialize() was called by FlucaInitialize()");
