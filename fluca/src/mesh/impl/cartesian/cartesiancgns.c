...
PetscErrorCode MeshView_CartesianCGNS(Mesh mesh, PetscViewer viewer) {
    ...
    PetscCallCGNS(cg_base_write(cgns->file_num, "Base", dim, dim, &cgns->base));

    {
        ...
        for (d = 0; d < dim; d++) {
            ...
            size[dim + d] = cart->N[d]; /* Number of elements */
        }
...

    {
        ...
        PetscCall(MeshCartesianGetCorners(mesh, &s[0], &s[1], &s[2], &m[0], &m[1], &m[2]));
        PetscCall(MeshCartesianGetIsLastRank(mesh, &is_last_rank[0], &is_last_rank[1], &is_last_rank[2]));

        for (d = 0; d < dim; d++) {
            /* Vertex ownership; note that CGNS uses 1-based index */
            rmin[d] = s[d] + 1;
            rmax[d] = s[d] + m[d] + is_last_rank[d];
        }
