...
PetscErrorCode PhysINSCreateSolverData_Internal(Phys phys)
{
  ...
  if (!ins->has_pressure_outlet) {
    ...
    /* Compose the pressure-only null space onto is_p so that PCFieldSplit auto-propagates it to
       the Schur complement diagonal sub-block, where the KSP projects it out of the RHS. */
    {
