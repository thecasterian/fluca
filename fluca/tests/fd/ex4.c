...
int main(int argc, char **argv)
{
  ...
  for (c = 0; c < ncols; ++c) {
    if (col[c].c < 0)
      PetscCall(
        PetscPrintf(PETSC_COMM_WORLD, "  col[%" PetscInt_FMT "]: i=%" PetscInt_FMT ", j=%" PetscInt_FMT ", loc=%s, c=%s_boundary, v=%g\n", c, col[c].i, col[c].j, DMStagStencilLocations[col[c].loc], FlucaFDBoundaryNames[FLUCAFD_BOUNDARY_FACE(col[c].c)], v[c]));
