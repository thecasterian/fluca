...
int main(int argc, char **argv)
{
  ...
  PetscCall(FlucaFDGetStencil(fd_comp, idx[0], idx[1], 0, &ncols, points));
  ...
  for (c = 0; c < ncols; ++c) {
    if (points[c].type == FLUCAFD_STENCIL_BOUNDARY)
      ...
      PetscCall(PetscPrintf(PETSC_COMM_WORLD, "  col[%" PetscInt_FMT "]: i=%" PetscInt_FMT ", j=%" PetscInt_FMT ", loc=%s, c=%" PetscInt_FMT ", v=%g\n", c, points[c].i, points[c].j, DMStagStencilLocations[points[c].loc], points[c].c, points[c].v));
  }
