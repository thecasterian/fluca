...
int main(int argc, char **argv)
{
  ...
  PetscCall(FlucaFDGetStencil(fd_sum, idx[0], idx[1], idx[2], &npoints, points));
  ...
  for (c = 0; c < npoints; ++c) {
    if (points[c].type == FLUCAFD_STENCIL_BOUNDARY)
      PetscCall(PetscPrintf(PETSC_COMM_WORLD, "  points[%" PetscInt_FMT "]: i=%" PetscInt_FMT ", j=%" PetscInt_FMT ", k=%" PetscInt_FMT ", loc=%s, c=%s_boundary, v=%g\n", c, points[c].i, points[c].j, points[c].k, DMStagStencilLocations[points[c].loc],
                            FlucaFDBoundaryNames[points[c].boundary_face], points[c].v));
    else
      PetscCall(PetscPrintf(PETSC_COMM_WORLD, "  points[%" PetscInt_FMT "]: i=%" PetscInt_FMT ", j=%" PetscInt_FMT ", k=%" PetscInt_FMT ", loc=%s, c=%" PetscInt_FMT ", v=%g\n", c, points[c].i, points[c].j, points[c].k,
                            DMStagStencilLocations[points[c].loc], points[c].c, points[c].v));
  }
...
/*TEST
  ...
  test:
    ...
    nsize: 1
    args: -stag_boundary_type_z periodic -x_flucafd_deriv_order 2 -y_flucafd_deriv_order 2 -z_flucafd_deriv_order 2 -k 0
